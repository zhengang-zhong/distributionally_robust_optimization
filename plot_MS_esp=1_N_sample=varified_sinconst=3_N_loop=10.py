...
Nt = np.shape(plot_traj_ave_list[0][::2])[0]
...
for plot in plot_traj_ave_list:
    plot_x_list += [plot[::2]]
    plot_v_list += [plot[1::2]]

N_plot = len(plot_x_list)

colors = pl.cm.viridis(np.linspace(0,0.7,N_sample_range))
...
for i in range(N_plot):
    ax1.plot(t_plot, plot_x_list[i], label=r'{\fontsize{14}{8}\selectfont} $N =' + str(i+1)+'$', color=colors[N_sample_range-1-i])
...
for i in range(N_plot):
    ax2.plot(t_plot, plot_v_list[i], label=r'{\fontsize{12}{6}\selectfont} $N$' + str(i+1), color=colors[N_sample_range-1-i])
...
fig.savefig("/Users/zhengangzhong/Dropbox/PhD/documents/paper_writing/CDC2021/result/plot2.pdf",
          bbox_inches='tight')
